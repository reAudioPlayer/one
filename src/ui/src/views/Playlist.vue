<!--
  - Copyright (c) 2023, reAudioPlayer ONE.
  - Licenced under the GNU General Public License v3.0
  -->

<template>
    <div class="playlist">
        <AddSong ref="addSongPopup" @close="updatePlaylist"/>
        <EditPlaylist
            ref="editPlaylistPopup"
            :playlist="{
                name: playlistName,
                description: playlistDescription,
                cover: playlistCover,
                id
            }"
            @close="updatePlaylist"/>
        <fixed-playlist-header
            ref="fixedHeading"
            :class="{ 'hidden': fixedHeaderHidden }"
            :title="playlistName"
            @loadPlaylist="loadPlaylist"
        />
        <div v-observe-visibility="headerVisibilityChanged" class="padding-20 playlisteditor">
            <Cover :src="playlistCover" type="playlist" />
            <div class="details">
                <div class="flex flew-row items-center">
                    <span
                        class="text-5xl cursor-pointer material-symbols-rounded ms-fill my-auto"
                        @click="loadPlaylist"
                    >
                        play_circle
                    </span>
                    <h1
                        class="font-black text-5xl ml-4"
                    >
                        {{ playlistName }}
                    </h1>
                </div>
                <h6 v-if="playlistDescription">{{ playlistDescription }}</h6>
                <div class="features flex flex-row gap-4 mt-4">
                    <FactCard
                        :primary-text="playlist.length"
                        :secondary-text="playlist.length === 1 ? 'Song' : 'Songs'"
                        class="w-full"
                    />
                    <FactCard
                        :primary-text="estimatedDuration"
                        class="w-full"
                        secondary-text="Total Duration"
                    />
                    <Card
                        class="p-4 w-full flex flex-col items-center justify-center"
                    >
                        <span id="addToPlaylist" class="material-symbols-rounded ms-fill" @click="addToPlaylist">add_circle</span>
                        <span class="text-muted">Add a Song</span>
                    </Card>
                    <Card
                        class="p-4 w-full flex flex-col items-center justify-center"
                    >
                        <span id="addToPlaylist" class="material-symbols-rounded ms-fill" @click="editPlaylist">edit</span>
                        <span class="text-muted">Edit This Playlist</span>
                    </Card>
                </div>
            </div>
        </div>
        <div class="mobileMenu showIfMobile">
            <span class="material-symbols-rounded" @click="() => $emit('toggleFullSidebar')">menu</span>
        </div>
        <hr>
        <div class="padding-20">
            <div class="grid">
                <PlaylistHeader
                    class="hideIfMobile"
                    with-album
                    with-more
                />
                <hr>
                <div class="playlistEntries">
                    <draggable v-model="playlist" @change="onPlaylistRearrange">
                        <template #item="{element}">
                            <PlaylistEntry
                                :index="playlist.findIndex(x => x.source == element.source)"
                                :playlist-id="Number(id)"
                                :selected="selectedSongId == element.id"
                                :song="element"
                                with-album
                                with-cover
                                with-more
                                @click="selectedSongId == element.id ? selectedSongId = -1 : selectedSongId = element.id"
                                @update="updatePlaylist"
                            />
                        </template>
                    </draggable>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
import FixedPlaylistHeader from "../components/Playlist/FixedPlaylistHeader.vue";
import PlaylistEntry from "../components/songContainers/PlaylistEntry.vue";
import PlaylistHeader from "../components/songContainers/PlaylistHeader.vue";
import AddSong from "../components/popups/AddNewSong.vue";
import EditPlaylist from "../components/popups/EditPlaylist.vue";
import draggable from "vuedraggable";
import Cover from "@/components/image/Cover.vue";

import { hashPlaylist, parseCover, parsePlaylistCover, unhashPlaylist } from "@/common";
import { usePlayerStore } from "@/store/player";
import { createPlaylist, getPlaylist } from "@/api/playlist";
import { useDataStore } from "@/store/data";
import FactCard from "@/containers/FactCard.vue";
import Card from "@/containers/Card.vue";

const data = useDataStore();


export default {
    components: {
        Card,
        FactCard,
        PlaylistEntry,
        FixedPlaylistHeader,
        PlaylistHeader,
        AddSong,
        EditPlaylist,
        draggable,
        Cover
    },
    name: 'Playlist',
    data() {
        this.updatePlaylist()

        return {
            fixedHeaderHidden: true,
            playlist: [],
            playlistName: "N/A",
            playlistDescription: "",
            playlistCover: null,
            store: usePlayerStore(),
            selectedSongId: -1,
        }
    },
    methods: {
        parseCover,
        parsePlaylistCover,
        download(index) {
            const data = this.playlist?.[index]
            window.open(`/api/tracks/${data.id}/download`)
        },
        onPlaylistRearrange(type) {
            const moved = type.moved

            if (!moved) {
                return;
            }

            fetch(`/api/playlists/${this.id}/tracks`, {
                method: "PUT",
                body: JSON.stringify({
                    songOldIndex: moved.oldIndex,
                    songNewIndex: moved.newIndex
                })
            })
        },
        headerVisibilityChanged(a) {
            this.fixedHeaderHidden = a
        },
        addToPlaylist() {
            this.$refs.addSongPopup.show();
        },
        editPlaylist() {
            this.$refs.editPlaylistPopup.show();
        },
        updateIsPlaying() {
            this.playlist.forEach((element) => {
                element.playing = element.id == this.currentSong
            })
        },
        updatePlaylist() {
            if (this.id == null) {
                return
            }
            if (!this.$route.path.includes("/playlist/")) {
                return;
            }

            if (this.$route.params.id == "create") {
                createPlaylist().then(id => {
                    const link = hashPlaylist(id);
                    this.$router.push(link);
                })
                return
            }
            getPlaylist(this.id).then(async jdata => {
                if (!jdata) {
                    this.$router.push("/")
                    return
                }

                this.playlist = jdata.songs
                this.playlistName = jdata.name
                this.playlistDescription = jdata.description
                this.playlistCover = jdata.cover
                document.title = `${this.playlistName} - reAudioPlayer One`;
                this.updateIsPlaying()
            })
        },
        loadPlaylist() {
            fetch("/api/player/load", {
                method: "POST",
                body: JSON.stringify({
                    id: Number(this.id),
                    type: "playlist"
                })
            })
        }
    },
    computed: {
        id() {
            return unhashPlaylist(this.$route.params.id)
        },
        autogeneratedDescription() {
            return `${this.playlist.length} ${this.playlist.length == 1 ? "song" : "songs"}${this.estimatedDuration}`
        },
        currentSong() {
            return this.store.song.id;
        },
        estimatedDuration() {
            let duration = 0;
            let isEstimate = false;

            if (!this.playlist.length) {
                return "";
            }

            for (const song of this.playlist) {
                isEstimate = isEstimate || ("-1:59" == song.duration);
                const songDurString = song.duration == "-1:59" ? "3:00" : song.duration; // estimate 3:00 if unknown
                const [min, sec] = songDurString.split(":")
                duration += Number(min * 60) + Number(sec);
            }

            const sec = duration;
            const min = Math.floor(sec / 60);
            const hs = Math.floor(min / 60);

            const prefix = isEstimate ? "about " : ""

            if (hs) {
                return prefix + `${hs} hr ${min - hs * 60} min`
            }

            if (min) {
                return prefix + `${min} min ${sec - min * 60} sec`
            }

            return prefix + duration + " sec";
        }
    },
    mounted() {
        this.updatePlaylist()
    },
    watch: {
        $route() {
            this.updatePlaylist()
        },
        currentSong() {
            this.updateIsPlaying()
        }
    }
}
</script>

<style lang="scss" scoped>
$mobileWidth: 950px;

.playlisteditor img {
    width: 15%;
    margin-right: 20px;
    border-radius: 5px;
}

.playlisteditor {
    display: flex;
    flex-direction: row;
    margin-bottom: 20px;
    margin-top: 5vh;

    @media screen and (max-width: $mobileWidth) {
        flex-direction: column;

        img {
            align-self: center;
            width: 40%;
        }
    }
}

.playlisteditor .details {
    flex: 1;
    display: flex;
    flex-direction: column;
    justify-content: flex-end;
}

#loadPlaylist,
#addToPlaylist {
    cursor: pointer;
    font-size: 3rem;
    width: 70px;
    line-height: 70px;
    text-align: center;
    align-items: center;
    vertical-align: middle;
    border-radius: 1000vmax;
    margin-bottom: 1.25rem;
    transition: all 0.2s ease-in-out;

    &:hover {
        background: var(--bg-hover-lt);
        color: var(--fg-secondary);
        transition: none;
    }
}

.padding-20 {
    padding: 20px;
}

h3 {
    text-transform: uppercase;
}

h7 {
    text-transform: uppercase;
    font-weight: bold;
    font-size: 0.83em;
}

h1 {
    margin-block-start: 0.15em;
    margin-block-end: 0.15em;
    font-size: 2.91em;

    @media screen and (max-width: $mobileWidth) {
        font-size: 1.4em;
        margin-top: 10px;
        margin-bottom: 0;
        margin-block-end: 0;
    }
}

.hidden {
    display: none !important;
}

p.muted.description {
    color: var(--font-darker);
    font-weight: normal;
    font-size: .7em;
    margin: 0;
}
</style>