...
const state = computed(() => {
    if (props.diff.removed.some(x => x.name === props.playlist.name)) {
        return "removed";
    }
    ...
    if (props.diff.added.some(x => x.name === props.playlist.name)) {
        return "added";
    }
...
const emit = defineEmits(["exclude", "toggle-expanded", "toggle-expanded-song"]);
...
<template>
    <Card :class="state" class="playlist p-4 rounded-xl relative">
        ...
        <div class="info">
            <h2 class="my-0">{{playlist.name}}</h2>
            <p
        ...
        <Card
            v-if="expanded"
            class="col-span-2 flex flex-col gap-2 z-10 p-4"
        >
            <SongDiff
...
.playlist {
    ...
    &:not(.base)::after, &:not(.base)::before {
        position: absolute;
        inset: 0 auto 0 0;
        padding: .25em;
        content: "";
