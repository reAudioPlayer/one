...
const formOptions = ref([{
    ...
    onChange: async (src: string) => {
        ...
        formOptions.value.find(x => x.name === "title").value = metadata.title;
        formOptions.value.find(x => x.name === "artist").value = metadata.artist;
        formOptions.value.find(x => x.name === "album").value = metadata.album;
        formOptions.value.find(x => x.name === "cover").value = metadata.cover;
        formOptions.value.find(x => x.name === "source").value = metadata.source;
    },
    value: song.source
}, {
    name: "title",
...
<template>
    <div class="downloader py-2 pr-2 grid gap-4 grid-cols-2 items-start">
        <div class="downloads">
            <Card
                v-for="state in downloader.states"
                :key="state.songId"
                :class="['song', { 'downloading': downloader.states[state.songId]?.status == 'downloading' }]"
                        ...
                        <div class="status">
                            ...
                            <template v-if="state.status !== 'error'">
                                ...
                                <div class="info">
                                    ...
                                    {{ state.status === "downloading" ? state.percent : 100 }}%
                                </div>
