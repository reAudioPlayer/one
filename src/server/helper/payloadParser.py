...
from typing import TypeVar, Type, Dict, Any, Callable, Awaitable, Union
from aiohttp import web
from pyaddict.schema import ISchemaType, ValidationResult, Object, ValidationError, OneOf


T = TypeVar("T")


def withObjectPayload(
    schema: Union[Object, OneOf], inBody: bool = False, inPath: bool = False, inQuery: bool = False
) -> Callable[
    [Callable[[Any, Any], Awaitable[web.Response]]], Callable[[Any, Any], Awaitable[web.Response]]
]:
    """decorator for parsing a payload"""
    assert inBody or inPath or inQuery, "at least one of inBody, inPath or inQuery must be True"

    def decorator(
        func: Callable[[Any, Any], Awaitable[web.Response]]
    ) -> Callable[[Any, Any], Awaitable[web.Response]]:
        @wraps(func)
...
class PayloadParser:
    ...
    @staticmethod
    async def fromBody(
        request: web.Request, schema: ISchemaType[Any], _: Type[T]
    ) -> ValidationResult[T]:
