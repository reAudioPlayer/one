...
class SongCache:
    ...
    @staticmethod
    def _songsToFiles(songs: Union[List[Song], IPlayerPlaylist]) -> List[str]:
        return [song.downloadPath() + ".mp3" for song in songs]
    ...
    @classmethod
    def forcePrune(cls, allow: Optional[Union[List[Song], IPlayerPlaylist]]) -> None:
        """force prune, even if preserve is True"""
        allowFiles = cls._songsToFiles(allow or [])
        toDelete = [file for file in cls.getAll() if file not in allowFiles]
        cls.deleteFiles(toDelete)

    @classmethod
    def prune(cls, allow: Optional[Union[List[Song], IPlayerPlaylist]]) -> None:
