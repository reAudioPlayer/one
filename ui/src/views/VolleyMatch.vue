...
import FullShelfSmallGrid from '../components/Catalogue/FullShelfSmallGrid.vue'
import Play from '../components/VolleyMatch/Play.vue'
import Stat from '../components/VolleyMatch/Stat.vue'
import Stats from '../components/VolleyMatch/Stats.vue'
import Team from '../components/VolleyMatch/Team.vue'
...
export default {
    ...
    methods: {
        updateData() {
            ...
            fetch(`/api/sports/volley/${this.$route.params.id}`)
                .then(async x => {
                    if (x.status == 400)
                    {
